...
3. **Production DB cutover (ADR-011 §4bis):** an explicit, owner-executed step (Azure Postgres provisioning + `alembic upgrade head` against the real `DATABASE_URL`). No Fase 5 PR provisions any cloud resource; the whole stack runs on SQLite by default until you cut over. Set `SNTO_API_KEY` when you want write-auth enforced (writes are open when it's empty).
4. **v2.0 — role-based UI evolution proper** (`docs/ux/ui-evolution-v2-spec.md`): the persistence gate is now lifted (Fase 5 foundations shipped; the "Acciones urgentes" tab is the first UI↔backend consumer). The broader role-based screen redesign is the next milestone — **not yet started**, by owner decision.
