...
La mayoría de los espacios naturales protegidos gestionan el impacto del turismo de forma **reactiva**: actúan cuando el problema ya es visible. El SNTO observa el estado de la vegetación desde satélite, deriva una **señal de cambio ambiental** (EHS/NDVI) a escala de huella de cada activo/sendero y **prioriza dónde investigar y hacer seguimiento o inspección de campo**. Hoy el sistema no dispone de una serie real de presión de visitantes ni de una campaña de validación de campo (#26): la señal satelital es *cambio ambiental observado*, **no** presión ni impacto turístico (la atribución de causa no está establecida), y el sistema no emite recomendaciones de gasto, cierre o restricción de acceso.

> **Ejemplo de decisión honesta (PNSG 2026/27):** con la evidencia ingerida hoy, la conclusión defendible para uso público es mayoritariamente *evidencia insuficiente para priorizar* — la señal S2 real 2021–2026 es en su mayoría estabilidad/reverdecimiento (6 reverdecen, 14 sin tendencia, 1 en declive), y no existe ningún dato de uso de visitantes a escala de activo. Ver el marco completo en **[docs/PNSG_DECISION_EVIDENCE_BRIEF.md](docs/PNSG_DECISION_EVIDENCE_BRIEF.md)**.

> SNTO **no reemplaza** a ArcGIS, Google Earth Engine, Sentinel Hub, Tableau ni Power BI: se sitúa **por encima** de las plataformas GIS, de observación de la Tierra y de BI, y traduce su señal en **prioridades de seguimiento e inspección de campo, con nivel de confianza explícito** — dentro del techo L1–L4 + L5a definido en el [contrato científico](docs/phase1/SCIENTIFIC_PRODUCT_CONTRACT.md) y la [matriz evidencia→decisión](docs/phase1/EVIDENCE_DECISION_MATRIX.md).
...
> **Para evaluadores y revisores:** este es un proyecto de software independiente desarrollado por su autor en el marco del Máster en Gestión y Planificación en Destinos Turísticos de la **Universidad Complutense de Madrid (UCM)** — no es un producto institucional de la UCM ni cuenta con aval institucional acreditado; la supervisión académica (Carmen Mínguez · Susana Ramírez García/REGENERA) se detalla en §12. Sobre el **Parque Nacional Sierra de Guadarrama** (218 senderos, cartografía oficial OAPN) el sistema procesa teledetección Sentinel-2 real y deriva índices de estado ambiental (EHS/NDVI); **hoy el techo de recomendación es seguimiento e inspección de campo (L5a)** — no atribución causal automatizada, no evaluación de eficacia ni cierre/restricción de acceso, gateados por la campaña de validación de campo pendiente (#26). El pipeline de inteligencia territorial de 7 fases, las fórmulas financieras y los escenarios de intervención (Pipeline B, ver §2) operan sobre **activos sintéticos calibrados**, no sobre datos reales del PNSG. El observatorio también incluye capas de **andamiaje temporal (serie 2021–2026), trazabilidad/confianza del dato, baselines estratificados, incertidumbre del ranking y herramientas de validación de campo aún sin campaña ejecutada**. **1058 tests passing, 1 skipped, CI separado del deploy, dos pipelines arquitectónicamente desacoplados.**
...
| **Expansión Red OAPN — piloto de replicabilidad (v1.2.0)** | Tablas de Daimiel (humedal, 5 activos) + Monfragüe (dehesa, 21 activos) | ✅ Series Sentinel-2 reales 2021–2026 **procesadas** (ingesta, EHS, Mann-Kendall) y en el selector de Tab 6; **sin validación de campo ni transferencia entre biomas confirmada** (#26); 13 parques restantes preparados como plantillas GEE, pendientes de la misma validación |
...
| **Pipeline A — Calibración metodológica** | Reserva de la Biosfera Sierra del Rincón (Madrid) | ✅ Pipeline ejecutado sobre escenas Sentinel-2 reales propias del territorio; **prueba de ejecución, no una validación del método en otro territorio** (sin campaña de campo, #26) |
...
| **v3.0 — Identidad y multi-tenancy** (`main`, dev, parcial) | — | 🟡 modelos `Organization`/`User`/`UserRole`, política `authz.py`, servicio de aprovisionamiento `tenancy.py`, `Territory.org_id` (aditivo, nulable) y gate de autorización en los 5 endpoints de escritura de `/api/v2`. **Latente hasta que existan `User` reales** — el modelo actual de API-key compartida no cambia. **SSO/Entra ID pendiente**; la **geometría PostGIS está implementada y probada en código (#133)** — espejo `managed_assets.geom` derivado y nulable con consultas `ST_DWithin`/`ST_Intersects`, head Alembic `b2c3d4e5f6a7` — **pero la migración aún no está aplicada en la base de datos de producción** y ningún consumidor usa las consultas espaciales |
...
El Pipeline A produce indicadores ambientales reales: el **PNSG** es el territorio principal del observatorio y la **Reserva de la Biosfera Sierra del Rincón** se conserva como piloto de calibración metodológica — **el pipeline se ha ejecutado sobre escenas Sentinel-2 reales de un segundo territorio, lo que no equivale a una validación de campo del método**. El Pipeline B demuestra el sistema de gobernanza de extremo a extremo sobre activos sintéticos. Ambos pipelines están diseñados para integrarse; el Pipeline A ya dispone de una serie temporal real 2021–2026 para 21 activos (v1.1.0, §9), pero la integración con el Pipeline B —aplicar su capa de gobernanza de 7 fases sobre activos reales del PNSG, en vez de los 20 activos sintéticos de Villuercas-Ibores-Jara— sigue pendiente. Desde v1.2.0, el método **se ha aplicado y ejecutado** en un piloto de dos biomas contrastados de la **Red de Parques Nacionales (OAPN)** (Tablas de Daimiel, Monfragüe) — series Sentinel-2 procesadas, sin validación de campo ni confirmación de transferencia entre biomas; el resto de la Red queda preparado como plantillas GEE para fases posteriores.
...
- **SCM (Spatial Causality Module)** clasifica cada activo en LOCALIZED_IMPACT / LANDSCAPE_DRIVEN / MIXED comparando el NDVI en tres zonas concéntricas (core 0–50 m / near 50–200 m / landscape 200–1000 m, EPSG:25830) mediante el Spatial Impact Gradient (SIG). **Hoy corre sobre una simulación de decaimiento α** — las zonas reales multiescala exportadas desde GEE aún no existen (`src/spatial_causality/zones/` vacío) — y se etiqueta como **hipótesis causal (evidencia SIMULATED, techo L2)**, no como una separación confirmada entre uso turístico y clima. Exportar las zonas reales cambiaría la **procedencia** del dato (SIMULATED → REAL), pero la clasificación seguiría siendo una **operación derivada/modelada**: el techo de afirmación permanece en **L2-hipótesis** — una atribución causal validada exige además la campaña de campo (#26), no solo el dato real.
- **DCS (Decision Confidence Score)** de 5 dimensiones (Data Quality, Temporal Robustness, Spatial Consistency, Model Stability, Signal Strength) con **data quality gate**: `can_act = False` si DQ < 10/25 o TR < 12/25, lo que bloquea incluso la recomendación de seguimiento/inspección (L5a). Superar el umbral **no autoriza gasto por sí mismo** — recomendar una intervención que compromete presupuesto exige evidencia ≥L6 (contrato científico §D/§E), hoy bloqueada por #26.
...
- **TPI (Territorial Priority Index)** para ranking de activos y **priorización de investigación/inspección** en 4 tiers (atención inmediata → promoción activa) — señala dónde mirar primero, no asigna presupuesto ni recursos por sí mismo.
...
- **Herramientas de validación de campo (campaña #26 pendiente) + pseudo-validación satélite↔satélite** — `src/validation/`: esquema de observación de campo (sin ejecutar) y métricas de concordancia (Spearman, contraste control-impacto BACI) que hoy solo pueden correr sobre puntos de control satelitales — una comprobación de consistencia interna, **no un sustituto** de la campaña de campo (#26). Ver [docs/field_validation_protocol.md](docs/field_validation_protocol.md).
...
- **Validación de campo:** el esquema y las métricas de concordancia (`src/validation/`) están listos; **falta la campaña de terreno** (penetrómetro, parcelas, control) — el gate #26. La pseudo-validación con puntos de control satelitales es una comprobación de consistencia interna, **no sustituye** la campaña de campo ni levanta el gate.
...
Proyecto de software independiente desarrollado por su autor en el marco del Máster en Gestión y Planificación en Destinos Turísticos de la **Universidad Complutense de Madrid (UCM)**; no es un producto institucional de la UCM. Supervisión académica: Carmen Mínguez · Susana Ramírez García (REGENERA).
...
<div align="center">
<sub>SNTO v2.1.0.dev0 · Python ≥ 3.12 · 1058 tests passing, 1 skipped · julio 2026</sub>
</div>
