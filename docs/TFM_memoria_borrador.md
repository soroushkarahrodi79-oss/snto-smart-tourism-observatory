...
[INSERTAR: número de senderos analizados, rango y distribución de EHS por activo, valores de ΔEHS estacional, activos con ΔEHS > 0 (alerta), clasificación SCM resultante por activo y presupuesto de restauración estimado. Estos valores proceden de la ejecución de `calculate_delta_ehs.py`, `run_scm_operational.py` y `tis_engine.py`.]

[INSERTAR: 1–2 figuras — mapa de EHS por sendero y/o histograma de ΔEHS.]
