...
| K-24 | **CANONICAL CORE RESOLVED — TRANSLATOR DIVERGENCE ACKNOWLEDGED, LEFT AS-IS BY OWNER DECISION.** Canonical EHS(health) condition partition (0/40/60/75/90 — CRITICAL/POOR/MODERATE/GOOD/EXCELLENT) | `src/risk_engine/ehs.py::EHS_CONDITION_BANDS` (bounds in `config/constants.py`), presentation in `platform/ehs_presentation.py` | `DECLARED_POLICY`, owner-approved pending validation (#26) | The core pure-EHS condition partition has been canonicalized: the dashboard map legends, real-trails priority bands, the spectral colour ramps, and the portfolio EHS-distribution summary all now derive from one source (`interpret_ehs()`, `real_trails.PRIORITY_BANDS`, the `map_layers`/`real_trails` spectral ramps, the `tab_diagnostic.py` legends, `territorial/portfolio.ehs_distribution()`). Old literal 0/30/45/60/75 and 0/30/45/60/75/85 copies removed. Tier (K-22) and `LEGEND_ITEMS` remain intentionally separate multi-factor / investment-priority systems — not unified with this partition, and not in scope. **Reviewed and closed as no-action:** `platform/translator.py::translate_ehs()` is a distinct, structurally different 6-tier EHS-only vocabulary (85/70/55/40/25, with an extra "DETERIORATING" state) that also embeds management-action/timing/promotion/closure prose alongside the condition label. It is **not reachable from the live app** — no reference from `app.py` or `src/ui/`, only invoked via the standalone `run_phase7_report.py` script — and has **no test coverage**. It also predates the Phase 1.0 claim-ladder gate and its prose (e.g. "the site should be closed to visitors," "emergency budget allocation is required") reads as L5b resource-committing/restrictive recommendation language, which is currently blocked pending #26 — a separate, larger scientific-policy question than a threshold reconciliation. The owner reviewed reconciliation options (align thresholds only / align + flag the L5b language / full rewrite) on 2026-08-31 and chose to **leave it untouched**, since rewriting unvalidated report-generation prose risks altering the output of an already-produced legacy artifact for no live-surface benefit. If this module is ever reconnected to a live surface, both the threshold divergence and the L5b language must be resolved before that happens. |
...
| **Duplicated meaning** | K-23 (alert thresholds ×2, merged); K-24 (EHS condition bands ×4, canonical core merged — see K-24 row); K-22 (Tier) and `LEGEND_ITEMS` remain intentionally separate, non-EHS systems; `platform/translator.py::translate_ehs()` is a structurally distinct 6-tier EHS vocabulary that also embeds management-action prose — reviewed and **left unmerged by owner decision** (2026-08-31), since it is unreachable from the live app, untested, and its closure/budget prose is a separate Phase 1.0 claim-ladder question, not a K-24 threshold question — see K-24 row |
