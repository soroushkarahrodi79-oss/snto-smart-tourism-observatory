...
- **Post-audit reclassification (Q-01).** Surfaces fed by
  `src/territorial/fixtures.py` were **initially classified as `CALIBRATED`
  during the audit**; the **owner decision after audit is `SYNTHETIC`**, and
  that decision governs Phase 0.5 and later work. The rows below carry the
  corrected class. This is a change in *how the audit classifies* the data —
  no machine-readable evidence field existed on fixture assets at audit time,
  and none exists today (making it so is Phase 0.5 item I-5). Under
  `evidence.py`'s gating matrix `SYNTHETIC` authorizes **no** decision use at
  all, so every `SYNTHETIC` row below is a surface presenting decision support
  that its own evidence class does not license.
...
| S-3 | Alert panel (sidebar counts) | `app.py:144-150` | Gestor | triage | `dashboard.kpis` statuses | Live | **SYNTHETIC** (Q-01) | Counts KPI statuses, not asset alerts — the label "indicador(es) crítico(s)" is accurate but easily read as "sites". Statuses derive from the fixture-fed KPIs. |
...
| S-5 | Dynamic territory banner | `render_widgets._render_banner` | Gestor | orientation | dashboard + counts | Live | **SYNTHETIC** (Q-01) | — |
| S-6 | Live-alerts strip + 60 s autorefresh | `app.py:180-185`, `render_widgets._render_live_alerts` | Gestor | urgency | `ranked_assets` | Live | **SYNTHETIC** (Q-01) | Pulsing "live" affordance over a dataset that only changes when an offline pipeline is re-run. See `SCIENTIFIC_CLAIMS_REGISTER.md` C-14. |
| S-7 | Executive KPI strip (4 tiles) | `app.py:227-238`, `render_widgets._compute_exec_kpis` | Gestor only | headline decisions | fixtures + budget (+ real SVI jobs) | Live | **MIXED**: REAL (jobs-at-risk tile, when the socio snapshot loads) × **SYNTHETIC** (the other three tiles, Q-01) | Only the "empleos en riesgo" tile is REAL; the remaining three are fixture-derived. Correctly captioned. |
...
|---|---|---|---|---|---|---|---|---|
| D-1 | **Panorama ejecutivo** — 10 KPI cards with meaning + recommended action | `ui/tabs/tab_kpis.py`, `platform/dashboard.py` | Gestor | portfolio triage, funding | 8 PNSG fixture assets | Live | **SYNTHETIC presented with REAL-grade confidence** (Q-01) | The 10 KPIs and all their narrative text derive from `fixtures.py` constants. Initially classified as CALIBRATED during the audit; owner decision after audit: SYNTHETIC — which under the gating matrix authorizes *no* decision use, making this the sharpest instance of the mismatch. See `KPI_INVENTORY.md`. |
| D-2 | Critical-asset cards + "ver ficha" | `tab_kpis.py:54`, `render_widgets` | Gestor | drill-down | fixtures | Live | **SYNTHETIC** (Q-01) | — |
...
| D-7 | **Impacto socioeconómico** — SVI, jobs at risk, ROI, per-municipality filter | `ui/tabs/tab_socioeco.py` | Gestor | justify investment | INE/ALMUDENA snapshot × fixture asset risk | Live (PNSG) | **MIXED: REAL demographics × SYNTHETIC fixture exposure** (Q-01) | The real INE/ALMUDENA figures are multiplied by fixture-derived asset exposure. The product is not REAL, and the tab's framing does not make the join explicit. Single dated snapshot → no trend. |

## 2. Layer *Diagnosticar* — "¿Es real la señal y dónde ocurre?"

| # | Feature | Code | Persona | Decision | Data | Status | Evidence | Limitations |
|---|---|---|---|---|---|---|---|---|
| G-1 | Context KPI grid (6 indicators) | `tab_diagnostic.py:61-73`, `ui/kpi_sections.py` | Técnica | context | fixtures | Live | **SYNTHETIC** (Q-01) | Re-renders 6 of the same 10 KPIs from D-1 — duplication by design ("trasladados desde el panorama"). |
...
| G-5 | Tier distribution / spectral legend + EHS stats | `tab_diagnostic.py:180-231` | all | interpret | fixtures | Live | **SYNTHETIC** (Q-01) | Two legends with **inconsistent band labels** (see `ARCHITECTURE_BASELINE.md` §4). |
| G-6 | **Sendas reales** — Pipeline A real-trail map, 5 KPIs, PRUG card, ranked table | `tab_diagnostic.py:246-442` | Técnica | prioritise real trails | `pipeline_a_results.geojson` (218 trails) | Live | **MIXED** — real trail geometry (OAPN cartography) · real-derived two-scene spectral health values · **SIMULATED** SCM attribution (`Causa (SCM)` column and tooltip; `src/spatial_causality/zones/` absent) · **derived/illustrative** management budget (modelled `length × cost/m × EHS × SCM factor`, not an observed cost) | The strongest evidence surface in the product, but **not uniformly REAL** — it presents a simulated causal attribution and a modelled budget in the same table as real measurements, with no visual or textual distinction (Phase 0.5 item I-4). Sits *below the fold* of a tab, subordinate to the fixture-driven map. Per Q-03 the two-scene difference may not be described as degradation, deterioration, recovery, seasonality or trend. |
...
| G-8 | **Catálogo de activos** + tier/type/region filters | `ui/tabs/tab_assets.py` | Técnica | browse, compare | fixtures + calibration | Live | **MIXED**: **SYNTHETIC** fixture attributes (Q-01) × real-derived EHS where the conservative satellite override fired | Three `multiselect` filters; ranking by TPI. The override subset is visible only via the calibration provenance column. |
| G-9 | **Presión y capacidad de carga** (LAC/ROS) | `ui/tabs/tab_portfolio.py:204-283`, `platform/lac_ros.py`, `pressure_capacity.py` | Gestor/Técnica | visitor management | `visitor_capacity_annual` (fixture) | Live | **SYNTHETIC** input (Q-01) under a `DECLARED_POLICY` LAC/ROS framework | Framework labelling is well-documented as a *planning estimate*; the pressure input it consumes is a fixture constant. `capacity_at_standard` additionally attributes **all** EHS deficit to visitor pressure — see `KPI_INVENTORY.md` K-19. |
...
| G-11 | Territorial pressure/risk matrix | `tab_portfolio.py:303-352` | Gestor | portfolio view | fixtures | Live | **SYNTHETIC** (Q-01) | — |
| G-12 | Active-alerts panel | `tab_portfolio.py:353-382` | Gestor | triage | fixtures | Live | **SYNTHETIC** (Q-01) | Duplicates S-6 and D-3 in a third place. |
...
| E-4 | **Confianza e incertidumbre (DCS)** + asset selector | `ui/tabs/tab_confidence.py`, `decision_confidence/assessor.py` | Auditoría | trust a recommendation | fixtures (`dcs` literal) | Live | **SYNTHETIC** (Q-01) | DCS is a *hard-coded field* on fixture assets, not computed at runtime for the dashboard portfolio — so a *confidence* score is itself synthetic. |
| E-5 | DCS decomposition + sensitivity charts | `tab_confidence.py:87-151` | Auditoría | improve evidence | fixtures | Live | **SYNTHETIC** (Q-01) | — |
| E-6 | Evidence-gap map | `tab_confidence.py:167` | Auditoría | where to measure | fixtures | Live | **SYNTHETIC** (Q-01) | Not a geographic map — a matrix. Name is misleading. |
...
| B-3 | **Informe ejecutivo** (.md / .json download) | `tab_reports.py:86-121`, `reporting/territorial_brief.py` | Gestor | share | fixtures | Live | **SYNTHETIC** (Q-01) | Correctly captioned "sin validación de campo (#26)", but this is a **downloadable, shareable** artifact built entirely on fixture constants — under the gating matrix `SYNTHETIC` authorizes no public reporting at all. |
...
| B-6 | **Seguimiento PRUG por zonas** (.md / .json) | `tab_reports.py:261-320`, `reporting/prug_monitoring.py` | Gestor/institutional | management-zone priorities | 218 real trails × OAPN zonification | Live (PNSG only) | **MIXED**: real OAPN zonification × real-derived two-scene spectral health, rolled up by a `DECLARED_POLICY` priority weighting | Correctly framed as *not* a compliance verdict, and the real-cartography × real-signal cross is genuine institutional value. **But the "seasonal early warning" framing does not hold under Q-03:** the underlying record is a single Aug-2025/Apr-2026 scene pair spanning two years and two sensors, which cannot support a seasonal interpretation. Rewrite as a dated two-scene difference. Degrades to `available=False` elsewhere. |
