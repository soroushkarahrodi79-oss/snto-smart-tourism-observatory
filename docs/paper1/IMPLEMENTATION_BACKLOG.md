...
> **✅ Resolved 2026-08-16 — no follow-ups remain on B-01 or A-4.** The owner supplied a Copernicus GLO-30 DEM (via OpenTopography) directly. `scripts/paper1/reproject_dem_to_utm.py` reprojected it EPSG:4326 → EPSG:25830 (verified deterministic, byte-identical on re-run) — necessary because computing aspect directly in geographic coordinates at this latitude skews the N/S split by the ~1.3× ratio between a degree of longitude and a degree of latitude here. `build_ecological_strata.py --dem-authoritative` classified all 218 trails (0 unresolved): 161 S2, 38 S1, 17 S3, 2 S4 — elevations sanity-checked against Peñalara's real summit (2428 m; DEM max 2424.2 m). The raw DEM is **not committed** (repo-wide raster policy, `.gitignore`); the derived `pnsg_trail_strata.csv` + its provenance sidecar (DEM SHA-256) are. Full details: `clean_assets/field_validation/reference/README.md`.
