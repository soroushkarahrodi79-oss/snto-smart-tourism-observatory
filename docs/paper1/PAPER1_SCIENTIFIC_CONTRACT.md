...
5. **Site-selection sub-constraint, confirmed 2026-08-09:** eligible segments are further restricted to the **Comunidad de Madrid-administered sector** of the park, per `PNSG_RESEARCH_AUTHORIZATION_REQUEST.md` §1 — filing one permit application instead of two. The frame is still "218 OAPN trails"; this narrows which of them Backlog B-01 draws from. The Madrid sector alone spans the full elevation/habitat gradient the four strata (S1–S4) need, so this does not weaken the design.
