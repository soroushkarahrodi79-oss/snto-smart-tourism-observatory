...
**Engineering (safe, unblocked, small):** ✅ **B-04 (plot↔cell matching) — done 2026-08-09**, `src/validation/spatial_match.py`, 24 tests · ✅ **B-06 (acquisition manifest) — done 2026-08-09**, `clean_assets/paper1/acquisition_manifest.json` at `status=planned`, 25 tests · ✅ **B-08 (SCM docstring reframing) — done 2026-08-09**, docstring-only "spatial contrast, not causal attribution" caveat in both SCM modules, 5 tests · B-01 site/plot generation now unblocked (needs B-04 ✅ + the Madrid boundary filter) · B-09 partial (Figure 1b, Figure 2, Table T2 from committed data) · B-14 (fresh SIG extraction for H4) once the manifest advances past `planned`. Full suite green (1488 passed).
