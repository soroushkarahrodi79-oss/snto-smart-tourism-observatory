...
| **G2 — Permit** | PNSG research authorisation obtained | **STOP.** No fieldwork without it. Content-complete 2026-08-16 (`PNSG_RESEARCH_AUTHORIZATION_REQUEST.md`) — identity, contact, CV/aval científico all assembled; blocked only on the owner physically filing with the Comunidad de Madrid (this assistant has no portal access/credential to do so) |
...
2. ✅ **PNSG research authorisation — content-complete 2026-08-16.** `PNSG_RESEARCH_AUTHORIZATION_REQUEST.md` has the full technical case (memoria), applicant identity/contact, and a tailored scientific-endorsement declaration, all assembled and delivered to the owner directly (kept out of this public repo). Territorial scope: **Madrid only**, real boundary applied (B-01). Submission strategy: zone-level, now (§5). **The only remaining step is the owner filing it with the Comunidad de Madrid** — this assistant cannot do that (no sede electrónica access, no digital certificate). Nothing has been submitted yet.
...
**Engineering (safe, unblocked, small):** ✅ **B-04 (plot↔cell matching) — done 2026-08-09**, `src/validation/spatial_match.py`, 24 tests · ✅ **B-06 (acquisition manifest) — done 2026-08-09**, `clean_assets/paper1/acquisition_manifest.json` at `status=planned`, 25 tests · ✅ **B-08 (SCM docstring reframing) — done 2026-08-09**, docstring-only "spatial contrast, not causal attribution" caveat in both SCM modules, 5 tests · ✅ **B-09 partial — done 2026-08-09**: Figure 1b (trail network × EHS), Figure 2 (pipeline schematic), Table T2 (live-valued constants) generated from committed data under `docs/paper1/figures|tables/`, 10 tests · ✅ **B-01 plot-plan engine — done 2026-08-09**: `scripts/paper1/generate_plot_plan.py` produces distinct, SM-1/2/3-validated impact/control coordinates + GPX · ✅ **A-4 ecological strata — done 2026-08-16**: owner supplied a real Copernicus GLO-30 DEM; reprojected to EPSG:25830 (`scripts/paper1/reproject_dem_to_utm.py`, verified deterministic) and classified all 218 trails (161 S2 / 38 S1 / 17 S3 / 2 S4, 0 unresolved, elevations sanity-checked against Peñalara's real summit) · ✅ **B-01 Madrid boundary finalised — 2026-08-15**: owner supplied the real Comunidad de Madrid boundary (OpenStreetMap relation 349055, 6 294 vertices), validated against known landmarks (La Pedriza in / Valsaín out) and repaired for a minor topology defect · ✅ **B-01 now stratified by real ecology, not the tercile fallback** — `clean_assets/field_validation/pnsg_plot_plan.csv` regenerated with `--boundary-authoritative --strata`: 41/218 trails eligible, 22 pairs, 44 plots, independently re-verified (0 SM-1/2/3 violations) · B-14 (fresh SIG extraction for H4) once the manifest advances past `planned`. Full suite green (1536 passed).
