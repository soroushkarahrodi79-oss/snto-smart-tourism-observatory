...
**Engineering (safe, unblocked, small):** ✅ **B-04 (plot↔cell matching) — done 2026-08-09**, `src/validation/spatial_match.py`, 24 tests · ✅ **B-06 (acquisition manifest) — done 2026-08-09**, `clean_assets/paper1/acquisition_manifest.json` at `status=planned`, 25 tests · ✅ **B-08 (SCM docstring reframing) — done 2026-08-09**, docstring-only "spatial contrast, not causal attribution" caveat in both SCM modules, 5 tests · ✅ **B-09 partial — done 2026-08-09**: Figure 1b (trail network × EHS), Figure 2 (pipeline schematic), Table T2 (live-valued constants) generated from committed data under `docs/paper1/figures|tables/`, 10 tests · ✅ **B-01 plot-plan engine — done 2026-08-09**: `scripts/paper1/generate_plot_plan.py` produces distinct, SM-1/2/3-validated impact/control coordinates + GPX · ✅ **A-4 ecological strata engine — done 2026-08-10**: `scripts/paper1/build_ecological_strata.py` derives S1–S4 from a DEM (elevation + N/S aspect), wired into B-01 via `--strata`; **blocked only on supplying a DEM** (all DEM sources proxy-blocked here) · ✅ **B-01 Madrid boundary finalised — 2026-08-15**: owner supplied the real Comunidad de Madrid boundary (OpenStreetMap relation 349055, 6 294 vertices), validated against known landmarks (La Pedriza in / Valsaín out) and repaired for a minor topology defect; `clean_assets/field_validation/pnsg_plot_plan.csv` regenerated with `--boundary-authoritative` — **no longer PROVISIONAL** (41/218 trails eligible, 22 pairs, 44 plots) · B-14 (fresh SIG extraction for H4) once the manifest advances past `planned`. Full suite green (1536 passed).
