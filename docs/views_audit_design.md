...
> **Nota de contexto (rescate #28, 2026-07).** Esta auditoría se escribió sobre
> el `app.py` monolítico (~3.000 líneas). Desde entonces la Fase 4 (issue #27)
> modularizó el dashboard: la lógica de vistas vive ahora en `src/ui/` (shell en
> `src/ui/layout.py`, una tab por fichero en `src/ui/tabs/`). Las referencias de
> abajo a "un archivo de ~3.000 líneas" y a "10 puntos dispersos" describen el
> estado previo; las fases F10 se implementan sobre la estructura modular actual.
> El diagnóstico de **qué** debe diferir por vista sigue vigente.
